...
*ʙᴏᴛ ɴᴀᴍᴇ : ${botName}*
*ᴠᴇʀsɪ : 𝚅𝟷.𝟻 𝙶𝙴𝙽 𝟷 𝚅𝙸𝙿*
*ᴍᴏᴅᴇ : ${client.public ? "𝙿𝚞𝚋𝚕𝚒𝚌":"𝙿𝚛𝚒𝚟𝚊𝚝𝚎"}*
*ᴅᴇᴠᴇʟᴏᴘᴇʀ : ${OwnerName}*
...
{ buttonId: ".allmenu", 
buttonText: { 
displayText: '𝙰𝙻𝙻 𝙼𝙴𝙽𝚄' }},
{ buttonId: ".bugmenu",
buttonText: {
displayText: '𝙱𝚄𝙶 𝙼𝙴𝙽𝚄' }
 }
],
viewOnce: true,
  headerType: 6
...
case 'ss':
case 'ssweb':{
if (!text) return replykymi(`mana linknya kak?`)
try {
client.sendMessage(m.chat, { image: { url: `https://skizoasia.xyz/api/ssweb?type=mobile&url=${encodeURIComponent(text)}&apikey=nonogembul` }}, { quoted: m})
			} catch {
	  reply('yah Error kak laporankan ke owner agar di perbaiki')
	}
}
case 'request': case 'reportbug': {
if (!text) return replykymi(`Contoh : ${prefix + command} hi dev play command is not working`)
textt = `*| REQUEST/BUG |*`
teks1 = `
